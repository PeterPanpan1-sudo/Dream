...
function LoginPage({ onLogin, onEmailLogin, theme, onThemeChange }) {
  ...
  const headerMap = {
    login: { label: 'SIGN IN', title: '欢迎回来', subtitle: '回到你的创作空间，继续解锁灵感。' },
        ...
        <div className="login-card">
          ...
          {authMode === 'login' && (
            <div className="login-mode-switch">
              <p className="mode-hint">选择你的登录方式</p>
              <div className="mode-tabs">
                ...
                <button
                  className={\`mode-tab \${loginType === 'email' ? 'active' : ''}\`}
          ...
          {authMode === 'register' && renderEmailForm('注册并登录')}
          ...
          {authMode === 'forgot' && forgotStep === 2 && (
            <form onSubmit={handleResetPassword} className="login-form">
              ...
              <button type="button" className="back-to-login" onClick={() => { setForgotStep(1); resetFields() }}>
          ...
          <div className="login-footer">
            {authMode === 'login' && (
              <>
                ...
                <p className="login-hint">登录后同步绑定你的账号信息</p>
              </>
            )}
            {authMode === 'register' && (
              <p className="register-hint">
            ...
            {authMode === 'forgot' && (
              <p className="register-hint">
                记起密码了？<button type="button" className="register-link" onClick={() => switchMode('login')}>立即登录</button>
              </p>
            )}
          </div>
        </div>
...
fs.writeFileSync('D:/daydream-studio/frontend/src/pages/LoginPage.jsx', content, 'utf8');
console.log('Written LoginPage.jsx');

// Verify
const written = fs.readFileSync('D:/daydream-studio/frontend/src/pages/LoginPage.jsx', 'utf8');
const lines = written.split('\n');
let ok = true;
lines.forEach((line, i) => {
  if (line.trim().startsWith('[loginType') || line.trim().startsWith('[forgotStep')) {
    console.log('STILL BROKEN at line', i+1);
    ok = false;
  }
});
if (ok) console.log('Verification OK');
