...
class OfficialImageClient {
  ...
  /**
   * 上传图片到 ChatGPT 文件服务 (参考 Go 版本 uploadImage)
   */
  async uploadImage(imageBuffer, fileName, contentType = 'image/png') {
    if (!imageBuffer || imageBuffer.length === 0) {
      throw new Error('图片数据为空');
    }

    // 1. 请求上传 URL 和 file_id
    const uploadPath = '/backend-api/files';
    const uploadPayload = {
      file_name: fileName,
    ...
    const uploadResp = await this.fetchUpstream(CHATGPT_API_BASE + uploadPath, {
      method: 'POST',
      headers: this.officialHeaders(uploadPath, {}, null, 'application/json'),
      body: JSON.stringify(uploadPayload)
    });

    if (!uploadResp.ok) {
      const text = await uploadResp.text();
      throw httpError('files_init', uploadResp.status, text);
    }
    ...
    if (!uploadUrl || !fileId) {
      throw new Error('文件上传元数据不完整');
    }

    console.log(`📤 上传图片到 Azure: ${fileId}`);

    // 2. PUT 上传图片数据到 Azure Blob
    const putResp = await this.fetchUpstream(uploadUrl, {
      method: 'PUT',
      headers: {
    ...
    if (!putResp.ok) {
      ...
      throw httpError('azure_upload', putResp.status, text);
    }

    // 3. 轮询检查文件可用性 (指数退避)
    for (let attempt = 0; attempt < 5; attempt++) {
      if (attempt > 0) {
        await sleep(50 * (1 << attempt));
      }

      const checkPath = `/backend-api/files/${fileId}`;
      try {
        const checkResp = await this.fetchUpstream(CHATGPT_API_BASE + checkPath, {
          method: 'HEAD',
          headers: this.commonHeaders(checkPath)
        });
    ...
    const finalizeResp = await this.fetchUpstream(CHATGPT_API_BASE + finalizePath, {
      method: 'POST',
      headers: this.officialHeaders(finalizePath, {}, null, 'application/json'),
      body: '{}'
    });

    if (!finalizeResp.ok) {
      const text = await finalizeResp.text();
      throw httpError('files_finalize', finalizeResp.status, text);
    }

    console.log(`✅ 图片上传完成: file-service://${fileId}`);
    return {
      fileId,
      fileName,
